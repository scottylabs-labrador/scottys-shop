/**
 * Chat component for displaying and interacting with a conversation
 * Provides message list, message input, and action buttons
 */
...
// Import components
import ConversationHeader from "@/components/conversations/chat/ConversationHeader";
import ConversationActionButtons from "@/components/conversations/chat/ConversationActionButtons";
import MessageList from "@/components/conversations/chat/Messages";
import MessageInput from "@/components/conversations/chat/MessageInput";
import ConversationStatusPreview from "@/components/conversations/chat/ConversationStatusPreview";
...
const Chat: React.FC<ChatProps> = ({ conversationId }) => {
  ...
  useEffect(() => {
    const fetchConversationData = async () => {
      ...
      try {
        ...
        if (!conversation) {
          // Update conversation status if we have it from the conversation object
          if (conversation) {
  ...
  const handleMarkAsCompleted = async () => {
    ...
    try {
      ...
      if (itemData.type === ITEM_TYPE.MARKETPLACE) {
  ...
  return (
    <div className="container mx-auto px-4 max-w-3xl">
      ...
      <ConversationHeader
        ...
        status={conversationStatus || CONVERSATION_STATUS.ONGOING}
        otherUser={otherUser}
        isSeller={isSeller}
          ...
          <>
            ...
            {itemData && (
              <ConversationStatusPreview
                status={conversationStatus || CONVERSATION_STATUS.ONGOING}
                itemType={
                  itemData.type === ITEM_TYPE.MARKETPLACE
