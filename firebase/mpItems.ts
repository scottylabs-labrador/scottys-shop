...
import { MPITEM_STATUS } from '@/utils/constants';
...
export const getMPItemById = async (itemId: string): Promise<MPItemWithId | null> => {
  try {
...
export const getMPItemsBySeller = async (sellerId: string): Promise<MPItemWithId[]> => {
  try {
...
export const getMPItemsByCategory = async (category: string): Promise<MPItemWithId[]> => {
  try {
    const q = query(
      mpItemsCollection, 
...
export const getMPItemsByPriceRange = async (minPrice: number, maxPrice: number): Promise<MPItemWithId[]> => {
  try {
    const q = query(
      mpItemsCollection,
...
export const updateMPItem = async (itemId: string, updates: Partial<MPItem>): Promise<void> => {
  try {
...
export const getLatestMPItems = async (limitCount: number = 10): Promise<MPItemWithId[]> => {
  try {
